<?php
require_once (__DIR__.'/models/usuario.php');
$web = new Usuario();
$web -> checar('Administrador');
require_once (__DIR__.'/views/header.php');
$accion = isset($_GET['accion']) ? $_GET['accion'] : null;
$id = isset($_GET['id']) ? $_GET['id'] : null;
$alerta = [];

switch ($accion){

    case 'crear':
        if(isset($_POST['enviar'])){
            $datos = $_POST['datos'];
            $resultado = $web -> crear($datos);
            if($resultado){
                $alerta ['mensaje'] = 'USUARIO CREADO CORRECTAMENTE';
                $alerta ['tipo'] = 'success';
                $usuarios = $web -> leer();
                $web -> alerta($alerta);
                include(__DIR__.'/views/usuario/index.php');
            }else{
                $alerta ['mensaje'] = 'NO SE PUEDE CREAR EL USUARIO';
                $alerta ['tipo'] = 'danger';
                $web -> alerta($alerta);
                include (__DIR__.'/views/usuario/form.php');
            }
        }else{
            include(__DIR__.'/views/usuario/form.php');
        }
        break;

    case 'modificar':
        $info = null;
        $info = $web -> leerUno($id);
        if(isset($_POST['enviar'])){
            $datos = $_POST['datos'];
            $resultado = $web -> modificar($datos,$id);

            if($resultado){
                $alerta ['mensaje'] = 'USUARIO MODIFICADO CORRECTAMENTE';
                $alerta ['tipo'] = 'success';
                $usuarios = $web -> leer();
                $web -> alerta($alerta);
                include(__DIR__.'/views/usuario/index.php');
            }else{
                $alerta ['mensaje'] = 'NO SE PUEDE MODIFICAR EL USUARIO';
                $alerta ['tipo'] = 'danger';
                $web -> alerta($alerta);
                include (__DIR__.'/views/usuario/form.php');
            }
        }else{
            include(__DIR__.'/views/usuario/form.php');
        }
        break;

    case 'eliminar':
        $resultado = $web -> eliminar($id);
        if($resultado){
            $alerta ['mensaje'] = 'USUARIO ELIMINADO CORRECTAMENTE';
            $alerta ['tipo'] = 'success';
        }else{
            $alerta ['mensaje'] = 'NO SE PUEDE ELIMINAR EL USUARIO, HAY ELEMENTOS QUE DEPENDEN DE EL';
            $alerta ['tipo'] = 'danger';
        }
        $web -> alerta($alerta);
        $usuarios = $web -> leer();
        require_once (__DIR__.'/views/usuario/index.php');
        break;

    case 'leer':
        default:
        $usuarios = $web -> leer();
        require_once (__DIR__.'/views/usuario/index.php');
}

require_once (__DIR__.'/views/footer.php');

?>